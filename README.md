...
- **04 버튼 4개 연습:** GPIO 입력과 동시 누름 상태를 게임 전에 확인
...
- 15초마다 템포 단계가 올라가며 줄 회전 속도가 점점 빨라집니다.
- 18초와 40초에는 반대 위상의 두 번째 줄이 추가되는 `DOUBLE ROPE` 이벤트가 각각 7초, 8초 동안 발생합니다.
- 네 명이 동시에 통과하면 팀 콤보와 FEVER 게이지가 쌓이고, 3콤보마다 7초간 획득 점수가 2배가 됩니다.
- 통과와 충돌은 캐릭터별 `CLEAR / MISS` 팝업, 파티클, 화면 흔들림으로 표시됩니다.
