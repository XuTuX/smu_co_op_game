...
│       ├── sound.js              # Web Audio API 효과음·게임별 배경음악 합성기
...
단체 줄넘기에서는 `W`, `S`, `A`, `D`가 각각 P1, P2, P3, P4의 점프 버튼입니다. 네 캐릭터는 각자 성공 횟수를 가지며 팀 공용 목숨 3개를 함께 사용합니다.
...
- 실패한 플레이어를 따로 표시하지 않고 팀 공용 목숨만 줄어듭니다.

### 타이밍 점프 규칙

- 리듬 패턴 없이 화살표와 `×개수`만 짧게 예고합니다.
- 왼쪽에서 오면 P1→P2→P3→P4, 오른쪽에서 오면 P4→P3→P2→P1 순서로 각자 점프합니다.
- 개인 목숨 대신 팀 공용 목숨 3개를 사용합니다. 충돌 뒤 2초 동안 팀 전체가 무적이어서 한 장애물에 목숨이 연속으로 빠지지 않습니다.
- 판정은 장애물의 앞쪽 모서리가 캐릭터 박스에 닿는 순간 이루어지며, 점프 직후 상승 동작도 정상 통과로 인정합니다.
- 충돌한 캐릭터 박스에만 약 0.8초 동안 빨간 테두리와 큰 `×`가 표시됩니다. 팀 무적 시간 중에는 목숨이 추가로 줄지 않더라도 실제로 닿은 캐릭터 표시는 각각 나타납니다.
- 1~3웨이브는 장애물 1개, 4~7웨이브는 2개, 이후에는 3개가 등장합니다.
- 같은 방향에서 연속 진입하거나 왼쪽→오른쪽 교차, 오른쪽 둘→왼쪽 하나처럼 순서가 달라집니다.
- 양쪽 교차는 동시에 충돌하지 않습니다. 첫 장애물이 팀을 모두 지난 뒤 착지·재점프 시간 1초 이상을 보장하고 반대쪽 장애물이 이어집니다.
- 주황 가시 묶음과 청록 회전 기어가 번갈아 나오며 후반에는 빠른 보라색 호버 로봇도 섞입니다.
- 네 플레이어는 장애물 피하기의 얼굴 달린 입체 박스 디자인을 함께 사용하며 노랑·빨강·파랑·초록 색상으로 구분합니다.
- 박스는 평상시·점프·성공·충돌 상태에 맞춰 기본 표정, 놀란 표정, 웃는 표정, 찡그린 표정으로 바뀌며 점프 성공 때 나타나던 흰색 번쩍임은 사용하지 않습니다.
- 생존 30초마다 장애물 속도가 추가로 올라가고 예고 시간과 웨이브 사이 휴식이 짧아집니다. 45초부터는 최소 2개, 90초부터는 3개 패턴이 유지되지만 장애물 사이는 항상 1초 이상의 착지 시간을 보장합니다.
...
- 플레이 중에는 게임별 배경음악이 재생되고 준비·종료 화면에서는 자동으로 멈춥니다. 우측 상단 소리 버튼으로 배경음악과 효과음을 함께 끄거나 켤 수 있습니다.
