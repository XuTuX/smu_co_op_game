...
장애물은 벽이나 한 줄 형태가 아니라 각각 독립된 `X` 박스로 등장합니다. 맵은 8열 × 9행이며 가로·세로 장애물은 모두 같은 크기입니다. 150점 전에는 한 번에 1개씩 생성되고, 150점부터는 1개 웨이브와 2개 동시 웨이브가 번갈아 나옵니다. 가로 장애물 2개는 서로 다른 줄에서 반대 방향으로 들어옵니다. 300점부터는 세로·가로 X 모두 각 박스마다 느림·보통·빠름 속도가 독립적으로 정해지고, 경고선이 먼저 표시된 뒤 레이저도 발사됩니다. 별은 느림·보통·빠름 중 하나의 속도로 맵 안을 곡선 이동하며 수시로 방향을 바꾸고 벽에 튕깁니다. 기본 점수는 생존 0.1초마다 +1점이며, 별을 획득하면 +10점입니다.
