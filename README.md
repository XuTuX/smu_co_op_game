...
> ESP32-S3가 `hihi` Wi-Fi와 웹서버를 직접 제공합니다. `hihi`에 연결한 뒤 `http://192.168.4.1`을 열면 Node.js 없이 게임과 GPIO 4 테스트 페이지를 사용할 수 있습니다. 개발 중에는 기존 Node.js 서버로 PC 테스트도 가능합니다.
...
- `server` 폴더에서 `npm install`, `npm start`를 실행하고 `http://localhost:3000`을 엽니다.
