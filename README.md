...
장애물은 벽이나 한 줄 형태가 아니라 각각 독립된 `X` 박스로 등장합니다. 위에서는 박스가 하나씩 곧게 내려오고, 좌우에서는 X 박스가 양방향으로 번갈아 이동합니다. 기본 점수는 생존 0.1초마다 +1점이며, 별을 획득하면 +10점입니다. 내부 난이도 3단계부터는 경고선이 먼저 표시된 뒤 레이저가 발사됩니다. 난이도 숫자는 플레이 화면에 표시하지 않습니다.
