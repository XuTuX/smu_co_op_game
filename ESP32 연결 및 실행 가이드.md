...
| 현재 버튼 | GPIO 4·5·6·7 버튼 4개 |
| 현재 모드 | `BUTTON_TEST_MODE 1` |
...
현재 버튼 테스트 모드에서는 루트 주소가 GPIO 4·5·6·7 전체 테스트 페이지를 표시합니다.

| 페이지 | 주소 |
|---|---|
| 버튼 4개 테스트 | `http://192.168.4.1` |
| 버튼 4개 테스트 직접 주소 | `http://192.168.4.1/button-test.html` |
...
4. GPIO 4, 5, 6, 7 버튼을 하나씩 누릅니다.
5. 해당 카드가 색상으로 바뀌고 `눌림 · LOW`가 표시되는지 확인합니다.
6. 버튼을 놓았을 때 `떼짐 · HIGH`로 돌아오는지 확인합니다.
7. 두 버튼 이상을 동시에 눌러 여러 카드가 함께 눌림 상태가 되는지 확인합니다.

상세 테스트 페이지에서는 다음 정보도 볼 수 있습니다.

- 각 버튼별 누른 횟수와 전체 누름 횟수
- 수신한 패킷 수
- 동시 입력 횟수
- 마지막 입력 시간
...
|---:|---|---|
| GPIO 4 | 전진 가속 | 위쪽 이동 |
| GPIO 5 | 후진 / 브레이크 | 아래쪽 이동 |
| GPIO 6 | 왼쪽 조향 | 왼쪽 이동 |
| GPIO 7 | 오른쪽 조향 | 오른쪽 이동 |

버튼 테스트 페이지에서는 단일 입력뿐 아니라 전진+왼쪽 같은 동시 입력도 확인할 수 있습니다.
...
```text
[BUTTON TEST] 4-button test page enabled: GPIO 4, 5, 6, 7
...
## 10. 기본 시작 페이지를 게임으로 변경

현재 네 버튼은 이미 GPIO 4·5·6·7에서 모두 동작합니다. `192.168.4.1`을 열었을 때 테스트 페이지 대신 버스 주차 게임을 기본으로 열려면 `src/main.cpp`에서 다음 값을 변경합니다.

```cpp
#define BUTTON_TEST_MODE 0
```

ESP32-S3 핀 배치는 변경되지 않습니다.
...
`BUTTON_TEST_MODE 0`에서는 `http://192.168.4.1`이 버스 주차 페이지를 기본 화면으로 엽니다. 테스트 페이지는 계속 `/button-test.html`에서 사용할 수 있습니다.
...
- [x] GPIO 4·5·6·7 버튼 테스트 모드 펌웨어를 적용함
...
- [ ] ESP32-S3 전원이 켜져 있음
- [ ] 네 버튼이 GPIO 4·5·6·7과 GND 사이에 연결됨
- [ ] PC 또는 휴대폰이 `hihi`에 연결됨
- [ ] 인터넷 없음 경고에도 `hihi` 연결을 유지함
- [ ] `http://192.168.4.1`을 HTTP로 열었음
- [ ] 화면에 ESP32 연결됨이 표시됨
- [ ] 네 버튼의 누름과 해제가 각각 화면에 반영됨
- [ ] 두 개 이상 버튼의 동시 입력이 화면에 반영됨
