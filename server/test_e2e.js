...
async function runTest() {
  ...
  const soundAsset = await new Promise((resolve, reject) => {
    http.get(`http://localhost:${TEST_PORT}/assets/sound/bus_Game_background.m4a`, (response) => {
      response.resume();
      response.on('end', () => resolve(response));
    }).on('error', reject);
  });
  if (soundAsset.statusCode !== 200 || !String(soundAsset.headers['content-type']).startsWith('audio/')) {
    throw new Error(`Expected bundled background music, got ${soundAsset.statusCode} ${soundAsset.headers['content-type']}`);
  }
  console.log('✅ TEST PASSED: bundled background music is served');
